...
function setMarkers(){
    ...
    for(let i = 0; i< tempJson.length;i++){
        ...
        if(geometry.length > 0 ){
            ...
            marker.bindPopup(`<p>${country}</p><p>Case:${cases}</p><p>Deaths:${deaths}</p><p>Recovered:${recoverd}</p>`).openPopup();
        }
    }
}
...
function displayStats(json){
    ...
    for (let i = 0; i < info.length; i++) {
        ...
        const cases = info[i][1];
        const deaths = info[i][2];
        const recoverd = info[i][3];
